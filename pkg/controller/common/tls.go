...
// This mirrors configv1.TLSAdherencePolicy from the pending openshift/api PR:
// https://github.com/openshift/api/pull/2680
// TODO(rhmdnd): Remove this type and use configv1.TLSAdherencePolicy once
// the openshift/api PR is merged and vendored.
type TLSAdherencePolicy string

const (
	TLSAdherencePolicyNoOpinion                             TLSAdherencePolicy = ""
	TLSAdherencePolicyLegacyExternalAPIServerComponentsOnly TLSAdherencePolicy = "LegacyAdheringComponentsOnly"
...
//
// This implements the same logic as the pending library-go PR:
// https://github.com/openshift/library-go/pull/2114
// TODO(rhmdnd): Remove this function and use
// libgocrypto.ShouldHonorClusterTLSProfile once the library-go PR is merged
// and vendored.
func ShouldHonorClusterTLSProfile(tlsAdherence TLSAdherencePolicy) bool {
	switch tlsAdherence {
	case TLSAdherencePolicyNoOpinion, TLSAdherencePolicyLegacyExternalAPIServerComponentsOnly:
		return false
	default:
		return true
	}
...
func extractTLSProfile(apiServer *configv1.APIServer) *configv1.TLSSecurityProfile {
	// TODO(rhmdnd): Once the openshift/api and openshift/library-go PRs are
	// merged and vendored, replace the local TLSAdherencePolicy type and
	// ShouldHonorClusterTLSProfile function with the upstream versions:
	//   - configv1.TLSAdherencePolicy (from openshift/api/pull/2680)
	//   - libgocrypto.ShouldHonorClusterTLSProfile (from openshift/library-go/pull/2114)
	//
	// The tlsAdherence field does not yet exist in the vendored APIServerSpec,
	// so we read it from an annotation as a temporary bridge. Once the field is
	// available, change this to:
	//   if !ShouldHonorClusterTLSProfile(TLSAdherencePolicy(apiServer.Spec.TLSAdherence)) {
	tlsAdherence := TLSAdherencePolicy(apiServer.Annotations["config.openshift.io/tls-adherence"])
	if !ShouldHonorClusterTLSProfile(tlsAdherence) {
