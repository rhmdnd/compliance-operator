...
func TestOperatorHonorsClusterTLSProfile(t *testing.T) {
	...
	// Old profile (TLS 1.0) so we can observe a measurable change in the
	// metrics endpoint TLS version.
	t.Log("Updating APIServer to strict adherence with Old TLS profile")
	apiServer, err = f.GetClusterAPIServer()
	if err != nil {
